...
$('#save').click(function(){
    ...
    success:function(response){
       	...
       	$('.modal').show()
       	$('.modal h4').after("<p>Link: <a href='" + window.location.origin + "/embed/#" + response.id + "'>" + window.location.origin + "/embed/#" + response.id +"</a></p>");
       	$('.modal h4').after("<p>iframe: &lt;iframe width='500px' height='300px' frameBorder='0' src='" + window.location.origin + '/embed/#' + response.id + "' &gt;&lt;/iframe&gt;</p>")
